  ...
  <div class="p-8 max-w-7xl">
    ...
    <div class="flex items-start justify-between mb-6">
      <div>
        ...
        <p class="text-muted-foreground text-sm">Google Search Console performance data for your sites.</p>
      </div>
      <div class="flex items-center gap-2">
        ...
        <button
          ...
          title="Connect or re-authorize Google Search Console"
        >
          <Loader2 v-if="authorizing" class="w-3.5 h-3.5 animate-spin" />
          <Link2 v-else class="w-3.5 h-3.5" />
          {{ authorizing ? 'Authorizing...' : 'Connect GSC' }}
        </button>
      </div>
    </div>
    ...
    <div v-else-if="!configured" class="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl p-5 mb-6">
      ...
      <div>
        <p class="text-sm font-semibold text-amber-900 mb-1">Google Search Console not configured</p>
        <p class="text-sm text-amber-800">
          Add a <code class="bg-amber-100 px-1 rounded">search_console</code> block to this site's YAML config
          (credentials_file, site_url) and run the pipeline once to authorize.
        </p>
      </div>
    </div>
    ...
    <div v-else-if="!authenticated" class="flex items-start gap-3 bg-blue-50 border border-blue-200 rounded-xl p-5 mb-6">
      ...
      <div class="flex-1">
        <p class="text-sm font-semibold text-blue-900 mb-1">GSC not authorized yet</p>
        <p class="text-sm text-blue-800 mb-3">
          Click <strong>Connect GSC</strong> to open the Google authorization page in your browser.
          Your token will be saved locally and this will only be needed once.
        </p>
        <button
          @click="authorizeGsc"
        ...
        >
          ...
          {{ authorizing ? 'Opening browser...' : 'Connect Google Search Console' }}
        </button>
    ...
    <!-- Auth result toast (when already authenticated, shows after re-auth) -->
    <div
    ...
    <template v-else-if="summary">
      ...
      <div class="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
        <div class="rounded-xl border border-border bg-card p-4">
          <div class="flex items-center gap-2 text-muted-foreground text-xs mb-1">
            <MousePointerClick class="w-3.5 h-3.5" /> Total Clicks
          </div>
          <p class="text-2xl font-bold text-foreground">{{ summary.total_clicks.toLocaleString() }}</p>
          <p class="text-xs text-muted-foreground mt-0.5">last {{ weeks * 7 }} days</p>
        </div>
        <div class="rounded-xl border border-border bg-card p-4">
          <div class="flex items-center gap-2 text-muted-foreground text-xs mb-1">
          ...
          <p class="text-xs text-muted-foreground mt-0.5">{{ summary.pages_tracked }} pages tracked</p>
        </div>
        <div class="rounded-xl border border-border bg-card p-4">
          <div class="flex items-center gap-2 text-muted-foreground text-xs mb-1">
            <Target class="w-3.5 h-3.5" /> Avg. Position
          </div>
          <p class="text-2xl font-bold text-foreground">{{ summary.avg_position }}</p>
          <p class="text-xs text-muted-foreground mt-0.5">across all pages</p>
        </div>
        <div class="rounded-xl border border-border bg-card p-4">
          <div class="flex items-center gap-2 text-muted-foreground text-xs mb-1">
            <TrendingUp class="w-3.5 h-3.5" /> Avg. CTR
          </div>
          <p class="text-2xl font-bold text-foreground">{{ summary.avg_ctr_pct }}%</p>
          <p class="text-xs text-muted-foreground mt-0.5">click-through rate</p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 xl:grid-cols-[2fr_1fr] gap-5 mb-6">
        ...
        <div class="rounded-xl border border-border bg-card p-5">
          <div class="flex items-center justify-between mb-4">
            <h2 class="text-sm font-semibold text-foreground">Clicks & Impressions Over Time</h2>
        ...
        <!-- Bar chart -->
        <div class="rounded-xl border border-border bg-card p-5">
          <h2 class="text-sm font-semibold text-foreground mb-4">Top 10 Pages by Clicks</h2>
          <div style="height: 220px">
      ...
      <div class="rounded-xl border border-border bg-card mb-6">
        <div class="px-5 py-4 border-b border-border flex items-center justify-between">
          <h2 class="text-sm font-semibold text-foreground">SEO Opportunities</h2>
          <div class="flex gap-1">
            ...
            >
              Low CTR ({{ ctrOpps.length }})
            </button>
          </div>
        </div>

        <div v-if="activeTab === 'page2' && !page2.length" class="px-5 py-8 text-center text-muted-foreground text-sm">
          No page 2 opportunities found.
        </div>
        <div v-else-if="activeTab === 'ctr' && !ctrOpps.length" class="px-5 py-8 text-center text-muted-foreground text-sm">
          No CTR opportunities found.
        </div>
        <div v-else class="overflow-x-auto">
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b border-border/50">
                <th class="px-5 py-2.5 text-left text-xs font-semibold text-muted-foreground">URL</th>
                <th class="px-4 py-2.5 text-right text-xs font-semibold text-muted-foreground">Clicks</th>
                <th class="px-4 py-2.5 text-right text-xs font-semibold text-muted-foreground">Impressions</th>
                <th class="px-4 py-2.5 text-right text-xs font-semibold text-muted-foreground">Position</th>
                <th class="px-4 py-2.5 text-right text-xs font-semibold text-muted-foreground">CTR</th>
              </tr>
            </thead>
            <tbody>
              <tr
      ...
      <div class="rounded-xl border border-border bg-card">
        <div class="px-5 py-4 border-b border-border">
          <h2 class="text-sm font-semibold text-foreground">Top Pages</h2>
        </div>
        <div class="overflow-x-auto">
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b border-border/50">
                <th class="px-5 py-2.5 text-left text-xs font-semibold text-muted-foreground">#</th>
                <th class="px-4 py-2.5 text-left text-xs font-semibold text-muted-foreground">URL</th>
                <th class="px-4 py-2.5 text-right text-xs font-semibold text-muted-foreground">Clicks</th>
                <th class="px-4 py-2.5 text-right text-xs font-semibold text-muted-foreground">Impressions</th>
                <th class="px-4 py-2.5 text-right text-xs font-semibold text-muted-foreground">Position</th>
                <th class="px-4 py-2.5 text-right text-xs font-semibold text-muted-foreground">CTR</th>
